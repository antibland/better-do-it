...
export async function POST() {
  try {
    ...
    console.log(`🚀 Starting schema completion migration in ${isProduction ? "production" : "development"}`);
    ...
    if (isProduction) {
      ...
      try {
        ...
        errors.push(`Failed to add addedtoactiveat column: ${error instanceof Error ? error.message : "Unknown error"}`);
      }
      ...
      try {
        ...
        try {
          ...
          if (error instanceof Error && error.message.includes("already exists")) {
            ...
            errors.push(`Failed to create partnership indexes: ${error instanceof Error ? error.message : "Unknown error"}`);
          }
        }
      } catch (error) {
        errors.push(`Failed to create partnership table: ${error instanceof Error ? error.message : "Unknown error"}`);
      }
      ...
      try {
        ...
        try {
          ...
          if (error instanceof Error && error.message.includes("already exists")) {
            ...
            errors.push(`Failed to create invite indexes: ${error instanceof Error ? error.message : "Unknown error"}`);
          }
        }
      } catch (error) {
        errors.push(`Failed to create invite table: ${error instanceof Error ? error.message : "Unknown error"}`);
      }
      ...
      try {
        ...
        const requiredTaskColumns = ['id', 'userid', 'title', 'iscompleted', 'createdat', 'completedat', 'isactive', 'addedtoactiveat', 'sort_order'];
        const existingTaskColumns = taskColumns.rows.map(row => row.column_name);
        
        const missingTaskColumns = requiredTaskColumns.filter(col => !existingTaskColumns.includes(col));
        
        if (missingTaskColumns.length > 0) {
          errors.push(`Missing task table columns: ${missingTaskColumns.join(', ')}`);
        } else {
          results.push(`Task table validation passed: ${existingTaskColumns.length} columns present`);
        }
      ...
      try {
        ...
        const taskColumns = appDb.prepare("PRAGMA table_info(task)").all() as Array<{ name: string }>;
        ...
        if (!existingTaskColumns.includes("addedToActiveAt")) {
          ...
          appDb.exec(`CREATE INDEX IF NOT EXISTS idx_task_addedToActiveAt ON task(userId, addedToActiveAt)`);
        ...
        try {
          ...
          `);
          appDb.exec(`CREATE INDEX IF NOT EXISTS idx_partnership_userA ON partnership(userA)`);
          appDb.exec(`CREATE INDEX IF NOT EXISTS idx_partnership_userB ON partnership(userB)`);
          results.push("Ensured partnership table and indexes exist");
        } catch (error) {
          results.push(`Partnership table setup: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
        ...
        try {
          ...
          `);
          appDb.exec(`CREATE INDEX IF NOT EXISTS idx_invite_code ON invite(code)`);
          appDb.exec(`CREATE INDEX IF NOT EXISTS idx_invite_inviterId ON invite(inviterId)`);
          appDb.exec(`CREATE INDEX IF NOT EXISTS idx_invite_inviteeEmail ON invite(inviteeEmail)`);
          appDb.exec(`CREATE INDEX IF NOT EXISTS idx_invite_status ON invite(status)`);
          appDb.exec(`CREATE INDEX IF NOT EXISTS idx_invite_expiresAt ON invite(expiresAt)`);
          results.push("Ensured invite table and indexes exist");
        } catch (error) {
          results.push(`Invite table setup: ${error instanceof Error ? error.message : "Unknown error"}`);
        }

      } catch (error) {
        errors.push(`SQLite migration failed: ${error instanceof Error ? error.message : "Unknown error"}`);
      }
    }

    // Final status report
    if (errors.length > 0) {
      console.error("❌ Schema completion migration completed with errors:", errors);
      return Response.json(
        {
          ...
          errors,
          results,
          message: "Schema completion completed with errors - manual intervention required",
