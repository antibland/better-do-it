...
export async function POST() {
  try {
    ...
    console.log(`🚀 Manually triggering schema initialization in ${isProduction ? "production" : "development"}`);
    ...
    if (isProduction) {
      ...
      try {
        ...
        errors.push(`PostgreSQL schema initialization failed: ${error instanceof Error ? error.message : "Unknown error"}`);
      }
      ...
      try {
        ...
        // Task indexes
        appDb.exec(`CREATE INDEX IF NOT EXISTS idx_task_user_isCompleted ON task(userId, isCompleted)`);
        appDb.exec(`CREATE INDEX IF NOT EXISTS idx_task_completedAt ON task(completedAt)`);
        appDb.exec(`CREATE INDEX IF NOT EXISTS idx_task_user_isActive ON task(userId, isActive)`);
        appDb.exec(`CREATE INDEX IF NOT EXISTS idx_task_addedToActiveAt ON task(userId, addedToActiveAt)`);
        appDb.exec(`CREATE INDEX IF NOT EXISTS idx_task_user_sortOrder ON task(userId, sortOrder)`);
        results.push("Ensured task table indexes exist");
        ...
        errors.push(`SQLite schema initialization failed: ${error instanceof Error ? error.message : "Unknown error"}`);
      }
    }
